#!/usr/bin/env bash
#
# Distributed MinIO baseline (reviewer Phase-2 follow-up): same as
# argo-tuned-minio.sh, but the bucket-clear step goes through the
# MinIO API (mc) rather than ssh+rm on a single hostPath. Works
# against any MinIO topology — single-pod or distributed — so the
# argo-bench bucket lives in a namespace-pinned cluster.
#
#   ./argo-distributed-minio.sh <argo_template> <out_dir> [N=10] [NS=e0-bench-dist]
# NOTE: deliberately NOT set -e — a transient kubectl-run failure during
# clear_bucket should not kill the entire 10-rep sweep. We check critical
# errors explicitly inside run_one.
set -uo pipefail

ARGO_TPL="${1:?usage: $0 <argo_template> <out_dir> [N=10] [NS=e0-bench-dist]}"
OUT="${2:?usage: $0 <argo_template> <out_dir> [N=10] [NS=e0-bench-dist]}"
N=${3:-10}
MINIO_NS=${4:-e0-bench-dist}

mkdir -p "$OUT"
SUM="$OUT/summary.csv"
[[ -f "$SUM" ]] || echo "rep,wf_name,phase,makespan_s,wall_s,bytes_in_total,bytes_out_total,report_md5" > "$SUM"

md5() { md5sum "$1" 2>/dev/null | awk '{print $1}'; }

# Reset the argo-bench bucket via mc. Spawning a fresh kubectl run per call
# is slow but unambiguous. Alternative: keep a long-lived helper pod.
# We just remove all objects and recreate the bucket if needed.
clear_bucket() {
    # Retry up to 3 times — `kubectl run` can transiently fail on busy
    # control planes and a single flake should not abort a 10-rep sweep.
    local attempt
    for attempt in 1 2 3; do
        local out
        out=$(kubectl -n "$MINIO_NS" run "mc-reset-$$-$attempt-$RANDOM" --rm -i --restart=Never \
            --image=minio/mc:latest --command --quiet -- sh -c "
mc alias set local http://minio:9000 e0admin e0adminpw >/dev/null 2>&1
mc rm --recursive --force local/argo-bench >/dev/null 2>&1 || true
mc mb -p local/argo-bench >/dev/null 2>&1 || true
echo done
" 2>&1 | grep -v "^If you\|^pod ")
        if echo "$out" | grep -q "done"; then
            return 0
        fi
        echo "  clear_bucket attempt $attempt failed: $out" >&2
        sleep 5
    done
    echo "  clear_bucket: giving up after 3 attempts; proceeding anyway" >&2
    return 0
}

wait_for_idle() {
    for i in $(seq 1 30); do
        local k=$(kubectl -n argo get pods --no-headers 2>/dev/null | grep -vE "Completed|Succeeded" | wc -l)
        [ "$k" = "0" ] && return 0
        sleep 5
    done
}

collect_argo_artifacts() {
    local rep=$1 wf=$2 dest=$3
    mkdir -p "$dest"
    kubectl -n argo get workflow "$wf" -o yaml > "$dest/workflow.yaml" 2>/dev/null
    kubectl -n argo get workflow "$wf" -o json \
      | python3 -c "
import json,sys
d=json.load(sys.stdin); out=[]
for nid,n in d['status'].get('nodes',{}).items():
    if n.get('type') != 'Pod': continue
    out.append({
      'task': n.get('displayName'),
      'node': n.get('hostNodeName'),
      'startedAt': n.get('startedAt'),
      'finishedAt': n.get('finishedAt'),
      'phase': n.get('phase'),
    })
print(json.dumps(out, indent=2))" > "$dest/placement.json"
    kubectl run probe-dist-$rep --rm -i --restart=Never --image=busybox \
      --overrides='{"spec":{"nodeName":"anrg-9","containers":[{"name":"p","image":"busybox","command":["sh","-c","cat /reports/'$wf'/report.json 2>/dev/null || cat /reports/unknown/report.json 2>/dev/null"],"volumeMounts":[{"name":"r","mountPath":"/reports"}]}],"volumes":[{"name":"r","hostPath":{"path":"/var/lib/wl-workloads/reports"}}]}}' 2>&1 \
      | grep -v "^If you\|^warning\|^pod " | python3 -c "
import sys; t=sys.stdin.read(); b=t.find('{'); e=t.rfind('}')+1; print(t[b:e] if b>=0 else '')
" > "$dest/report.json" || true
    sshpass -p anrg ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR anrg@anrg-9 \
        "echo anrg | sudo -S rm -rf /var/lib/wl-workloads/reports/unknown" </dev/null >/dev/null 2>&1 || true
}

run_one() {
    local rep=$1
    local dest="$OUT/rep${rep}-argo"
    echo "  clearing distributed MinIO bucket..."
    clear_bucket
    wait_for_idle
    local start=$(date +%s)
    local out
    out=$(kubectl -n argo create -f <(cat <<EOF
apiVersion: argoproj.io/v1alpha1
kind: Workflow
metadata:
  generateName: ${ARGO_TPL}-dist-
  namespace: argo
spec:
  workflowTemplateRef: { name: ${ARGO_TPL} }
EOF
) 2>&1)
    local wf=$(echo "$out" | sed -nE 's|workflow.argoproj.io/(.+) created.*|\1|p')
    if [ -z "$wf" ]; then
        echo "  ERROR: $out"
        echo "$rep,?,Failed,?,?,NA,NA,?" >> "$SUM"
        return
    fi
    echo "  submitted $wf"
    for j in $(seq 1 120); do
        sleep 15
        local p=$(kubectl -n argo get workflow "$wf" -o jsonpath='{.status.phase}' 2>/dev/null)
        case "$p" in Succeeded|Failed|Error) break;; esac
    done
    local end=$(date +%s)
    local wall=$((end - start))
    local ms=$(kubectl -n argo get workflow "$wf" -o json 2>&1 | python3 -c "
import json,sys
from datetime import datetime
d=json.load(sys.stdin); sa=datetime.fromisoformat(d['status']['startedAt'].replace('Z','+00:00')); fa=datetime.fromisoformat(d['status']['finishedAt'].replace('Z','+00:00'))
print(int((fa-sa).total_seconds()))" 2>/dev/null)
    local phase=$(kubectl -n argo get workflow "$wf" -o jsonpath='{.status.phase}' 2>/dev/null)
    collect_argo_artifacts "$rep" "$wf" "$dest"
    local h=$(md5 "$dest/report.json")
    echo "$rep,$wf,$phase,$ms,$wall,NA,NA,$h" >> "$SUM"
    echo "  -> rep $rep: phase=$phase makespan=${ms}s wall=${wall}s"
    kubectl -n argo delete workflow "$wf" --wait=false >/dev/null 2>&1 || true
}

echo "Distributed Argo+MinIO baseline: $ARGO_TPL, N=$N reps, MinIO ns=$MINIO_NS"
echo "Output: $OUT"
echo
for r in $(seq 1 "$N"); do
    echo "==================== rep $r ===================="
    run_one "$r"
done
echo
echo "##############################################"
echo "## SUMMARY  $SUM"
echo "##############################################"
column -t -s, < "$SUM"
echo
python3 - <<PY
import csv, statistics
rows = list(csv.DictReader(open("$SUM")))
ms = [int(r['makespan_s']) for r in rows if r['phase']=='Succeeded' and r['makespan_s'] not in ('','?')]
n = len(rows)
if ms:
    print(f"Argo distributed-MinIO reps: {ms}")
    print(f"mean={statistics.mean(ms):.1f}s  std={statistics.pstdev(ms):.1f}s  n_ok={len(ms)}/{n}")
PY
