#!/usr/bin/env bash
# E5 symmetric network: per-node egress HTB with three rate tiers,
...
#   ./e5_net.sh apply | verify | clear
set -uo pipefail
CMD=${1:?usage: e5_net.sh apply|verify|clear}
NS=wl-system
...
fw() { kubectl -n "$NS" exec "e5-fw-$1" -- sh -c "$2" 2>&1; }
