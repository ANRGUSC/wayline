#!/usr/bin/env python3
"""Freeze a reference schedule from the controller's OWN scheduling call.
...
Usage: freeze_from_live.py <template> <algo-short> <out-dir>
"""
import hashlib
import json
import os
...
def one(template, short, out_dir):
    ...
    for n, ts in sorted(order.items()):
        print(f"    {n:8} {' -> '.join(ts)}")
    gw = [t for t, n in placement.items() if n == "anrg-9"]
    if gw:
        raise SystemExit(f"STOP: application tasks on the gateway: {gw}")
    return rec
